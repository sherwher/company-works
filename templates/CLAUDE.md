...
본 문서는 사내 AI 에이전트 협업 시스템의 **단일 규약 템플릿**입니다.
신규 프로젝트는 이 파일을 자기 리포 루트에 `CLAUDE.md`로 복사하고 S1의 빈칸을 채우면 즉시 운영 가능합니다.

각 원칙의 근거는 본 템플릿이 관리되는 리포의 [`research/00-synthesis.md`](../research/00-synthesis.md)를 참조하세요. (복사 후 신규 프로젝트에서는 해당 링크를 제거하거나 사본을 함께 복사하세요.)
...
### 3.8 AI 엔지니어 (AI Engineer) - 조건부 참여
...
- **Deliverables**: AI Spec (S5.11) + 모델/파이프라인 코드 + PR
- **DoD**: 평가 메트릭 기준 충족 + 추론 지연/비용 SLO 충족 + 롤백 가능 + 모니터링(drift/성능) + PR 리뷰 통과
- **권한**: L2 (모델 배포/서빙은 L3, 학습 데이터 삭제는 L4)

### 3.9 사업전략 (Strategy) - 오프라인 참여
...
### 3.10 CS (Customer Support) - 오프라인 참여
...
| G2-AI | 기획 -> AI엔지니어 | AI 요구사항 포함 시: AI Spec 작성 착수 (S5.11) |
...
| AI 엔지니어 | 모델 변경 시 평가 회귀(eval regression) 필수. 학습 데이터 출처/라이선스 명시. 프로덕션 모델 교체는 L3. |
